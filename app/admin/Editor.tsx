...
function mdxParaBlocos(content: string): Bloco[] {
  ...
  while (restante.length > 0) {
    // 1. Imagem flutuante
    if (restante.startsWith('<div style="float:')) {
      const m = restante.match(/<div style="float:(left|right);[^"]*width:([^;"]+)[^"]*">\s*<img src="([^"]*)"[^>]*\/?>\s*(?:<p[^>]*>([^<]*)<\/p>)?\s*<\/div>\n?([\s\S]*?)<div style="clear:both"><\/div>/)
      if (m) {
        blocos.push({
          tipo: 'imagem-flutuante',
          lado: m[1] as 'left' | 'right',
          largura: m[2].trim(),
          src: m[3],
          legenda: m[4] || '',
          textoAoLado: m[5].trim(),
        })
        restante = restante.slice(m[0].length).trim()
        continue
      }
    }

    // 2. Galeria
    if (restante.startsWith('<div class="galeria-wrap"')) {
      const m = restante.match(/<div class="galeria-wrap"[^>]*>([\s\S]*?)<\/div>\n<script>[\s\S]*?<\/script>/)
      if (m) {
        const imagens: { src: string; legenda: string }[] = []
        const slideRe = /<img src="([^"]*)"[^>]*\/>(?:<p[^>]*>([^<]*)<\/p>)?/g
        let sm
        while ((sm = slideRe.exec(m[1])) !== null) {
    ...
    // 3. Imagem simples
    if (restante.startsWith('<figure')) {
      const m = restante.match(/<figure[^>]*>\s*<img src="([^"]*)"[^>]*\/?>\s*(?:<figcaption[^>]*>([^<]*)<\/figcaption>)?\s*<\/figure>/)
      if (m) {
        const alignMatch = m[0].match(/width:([\d.]+%?)/)
        const marginMatch = m[0].match(/margin:([^";\s]+)/)
        ...
        blocos.push({ tipo: 'imagem-simples', src: m[1], legenda: m[2] || '', alinhamento })
        restante = restante.slice(m[0].length).trim()
        continue
      }
    }
    ...
    // 4. Video YouTube
    if (restante.startsWith('<div style="position:relative;padding-bottom:56.25%')) {
      const m = restante.match(/<div style="position:relative;padding-bottom:56\.25%[^"]*">([\s\S]*?)<\/div>(?:\n\*([^*]*)\*)?/)
      if (m) {
        const iframeM = m[1].match(/embed\/([\w-]{11})/)
        if (iframeM) {
          blocos.push({ tipo: 'video', url: `https://www.youtube.com/watch?v=${iframeM[1]}`, legenda: m[2] || '' })
          restante = restante.slice(m[0].length).trim()
          continue
        }
      }
    ...
    const nextIdx = restante.search(/\n\n<(div|figure|script)/)
    if (nextIdx > 0) {
      blocos.push({ tipo: 'texto', conteudo: restante.slice(0, nextIdx).trim() })
      restante = restante.slice(nextIdx).trim()
    } else {
...
export default function Editor({ slug: slugProp }: { slug?: string }) {
  ...
  // Upload imagem simples â€” usa setBlocos diretamente para garantir re-render
  async function handleImagemSimples(idx: number, file: File) {
    setUploadingIdx(idx)
    const url = await uploadImagem(file)
    if (url) {
      setBlocos(prev => prev.map((b, i) => {
        if (i !== idx || b.tipo !== 'imagem-simples') return b
        return { ...b, src: url }
      }))
    }
    setUploadingIdx(null)
  ...
  // Upload galeria â€” usa setBlocos diretamente
  async function handleGaleriaUpload(idx: number, files: File[]) {
    setUploadingIdx(idx)
    ...
    if (novasUrls.length > 0) {
      setBlocos(prev => prev.map((b, i) => {
        ...
        return { ...b, imagens: [...b.imagens, ...novasUrls.map(src => ({ src, legenda: '' }))] }
      }))
    }
    setUploadingIdx(null)
  }
    ...
    <div style={s.page}>
      <header style={s.header}>
        ...
        <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
          {saved && <span style={{ color: '#3db87a', fontSize: '0.875rem' }}>âœ“ Salvo!</span>}
              ...
              <div key={idx + '-' + ('src' in bloco ? bloco.src : '') + '-' + ('imagens' in bloco ? bloco.imagens.length : '')}>
                  ...
                  <div style={s.blocoImagem}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                      ...
                      <div>
                        ...
                        <textarea style={{ ...s.input, minHeight: '200px', resize: 'vertical' as const, lineHeight: '1.7' }}
                          value={bloco.textoAoLado} onChange={e => updateBloco(idx, { textoAoLado: e.target.value })}
                  ...
                  <div style={{ ...s.blocoTexto, borderColor: '#4a9fd4' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '0.75rem' }}>
                      <div>
                        ...
                        <input style={s.input} value={bloco.url} onChange={e => updateBloco(idx, { url: e.target.value })} placeholder="https://www.youtube.com/watch?v=..." />
                      </div>
                ...
                {bloco.tipo === 'imagem-simples' && (
                  <div style={{ ...s.blocoTexto, borderColor: '#3db87a' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '0.75rem' }}>
                      {uploadingIdx === idx ? (
                        <div style={{ height: '120px', background: '#0a0a0a', borderRadius: '6px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#888' }}>
                          Enviando imagem...
                        </div>
                      ) : bloco.src ? (
                        <div style={{ position: 'relative' }}>
                      ...
                      <div style={{ display: 'flex', gap: '0.5rem' }}>
                        {(['full', 'center', 'left', 'right'] as const).map(a => (
                          <button key={a} onClick={() => updateBloco(idx, { alinhamento: a })} style={{
                            ...
                            {a === 'full' ? 'Total' : a === 'center' ? 'Centro' : a === 'left' ? 'Esq' : 'Dir'}
                          </button>
                        ))}
                      </div>
                      <div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '0.75rem' }}>
                      <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' as const }}>
                        ...
                        <label style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100px', height: '100px', background: uploadingIdx === idx ? '#111' : '#0a0a0a', border: '1px dashed #444', borderRadius: '4px', cursor: 'pointer', color: '#888', fontSize: '0.75rem', flexDirection: 'column' as const, gap: '0.25rem' }}>
                          {uploadingIdx === idx ? 'Enviando...' : '+ Foto'}
                          <input type="file" accept="image/*" multiple style={{ display: 'none' }} disabled={uploadingIdx === idx}
                            onChange={e => { const files = Array.from(e.target.files || []); if (files.length) handleGaleriaUpload(idx, files) }} />
                        </label>
                      </div>
